    ...
    <main className="pm-shell">
      ...
      <section className="pm-metrics" aria-label="Account summary">
        ...
        <Metric
          ...
          label="Audit target"
          value={selectedMarket?.status ? formatStatus(selectedMarket.status) : "none"}
        />
      </section>
    ...
    <article className={`pm-market ${selected ? "pm-selected" : ""}`}>
      ...
      <div className="pm-market-body">
        ...
        <div className="pm-market-stats">
          ...
          <span>{market.payoutPendingCount} pending</span>
          <span>{market.payoutSucceededCount} paid</span>
          <span>{market.payoutFailedCount} failed</span>
        </div>
  ...
  return (
    <section className="pm-panel">
      ...
      <div className="pm-compact-list">
        ...
        {payments.length === 0 && (
          <div className="pm-empty pm-small-empty">No payments</div>
        )}
      </div>
    </section>
  );
  ...
  return (
    <section className="pm-panel">
      <div className="pm-panel-head">
        <h2>Audit</h2>
        {isLoading ? (
          <Loader2 className="pm-spin pm-muted-icon" size={16} />
        ) : (
...
function AuditRow({ event }: { event: AuditEvent }) {
  return (
    <article className="pm-audit-row">
      <span>
        #{event.sequence} {formatStatus(event.eventType)}
      </span>
      <p>{event.message}</p>
    </article>
  );
