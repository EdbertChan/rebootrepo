...
PAYMENT_ATTEMPT_HISTORY_LIMIT = 10
...
async def _default_gateway_charge(
    ...
    return GatewayOutcome(
        ...
        message="Simulated gateway accepted payment.",
    )
...
class PaymentIntentServicer(PaymentIntent.Servicer):
    async def create(
        ...
        self.state.idempotency_key = f"payment-intent:{self.ref().state_id}"
    ...
    async def record_attempt(
        ...
        self.state.attempts = self.state.attempts[-PAYMENT_ATTEMPT_HISTORY_LIMIT:]
    ...
    async def run(cls, context: WorkflowContext) -> None:
        ...
        for attempt_number in range(len(payment.attempts) + 1, MAX_PAYMENT_ATTEMPTS + 1):
            ...
            outcome = await at_least_once(
                f"Charge payment attempt {attempt_number}",
                context,
                call_gateway,
            )
            await PaymentIntent.ref(payment_intent_id).per_workflow(
                f"Record payment attempt {attempt_number}"
            ).record_attempt(
                context,
