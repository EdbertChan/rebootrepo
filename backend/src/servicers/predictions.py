...
async def _insert_id(
    ...
    try:
        await OrderedMap.ref(map_id).create(context)
    except OrderedMap.CreateAborted:
        pass
...
async def _ordered_ids(
    ...
    *,
    ...
    except OrderedMap.RangeAborted:
        return [], ""
    ids = [entry.bytes.decode() for entry in response.entries]
    next_cursor = ""
    if len(response.entries) == page_size:
